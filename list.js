...
jQuery(function($) {
  jQuery( "#listing" ).html( '<img class="loading_icon" src="grey.gif" />' )
  var pageTitle = ""
  if ( typeof PAGE_TITLE != "undefined" )
  {
      pageTitle = PAGE_TITLE
  }
  else if ( typeof AUTO_TITLE != "undefined" && AUTO_TITLE == true )
  {
      pageTitle = location.hostname
  }

  var pageSubTitle = ""
  if ( typeof SUB_TITLE != "undefined" )
  {
      pageSubTitle = SUB_TITLE
  }
  else if ( typeof BUCKET_DISPLAY_URL != "undefined" )
  {
      pageSubTitle = BUCKET_DISPLAY_URL
  }

  jQuery( ".scheme" ).text( pageTitle );
  jQuery( ".author" ).text( pageSubTitle );

  setTimeout("getS3Data()",1300);
 });
...
function prepareTable(info) {
  ...
  jQuery.each(files, function(idx, item) {
    ...
    // console.log(jQuery.inArray( item.Key, EXCLUDE_ARRAY ));
    if (typeof EXCLUDE_ARRAY !== 'undefined' && jQuery.inArray( item.Key, EXCLUDE_ARRAY ) === -1)
      content.push(row + '\n');
    // else if (typeof EXCLUDE_FILE == 'undefined' || EXCLUDE_FILE != item.Key)
    //   content.push(row + '\n');
  });
